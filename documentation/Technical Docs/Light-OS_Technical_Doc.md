...
*   ✨ **L'IA écrit la suite** (`logic/proposerUnEffet.ts`) : même patron que le § 89 — schéma imposé au décodeur, `sansPersona`, et **rien n'est écrit sans accord**. La validation vit à part (`logic/effetPropose.ts`). ⛔ **Elle JETTE une étape dont la couleur n'est pas un hexadécimal**, là où `etapeBornee` la repeindrait en blanc : *une saisie humaine mal tapée se voit et se corrige ; un blanc glissé par un modèle au milieu d'un orage passe pour une intention.* ⚠️ Et un `alea` absent vaut **25**, pas zéro — une boucle parfaitement régulière ressemble à une machine.
