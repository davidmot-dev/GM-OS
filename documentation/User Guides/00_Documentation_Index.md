# 🗂️ Index de la Documentation GM-OS v6

Bienvenue dans le centre de documentation de **GM-OS v6**. Ce guide centralise toutes les ressources nécessaires pour maîtriser votre système d'exploitation de Maître de Jeu.

> [!IMPORTANT]
> **Source de Vérité v6** : Pour le suivi de la refonte actuelle, consultez la **[Roadmap v6 Master](../../docs/architecture/roadmap-v6.md)**.

---

## 🧠 Savoir & Retours d'Expérience
- [**Lessons Learned (Fichier Maître)**](../Lessons_Learned.md) : L'historique complet des défis techniques et solutions de v5 à v6.
- [**Guide de Migration & Standard de Code**](../../instructions.md) : Les règles d'or de l'architecture Bridge et du typage strict.
- [**Roadmap d'Amélioration**](../Plans/amélioration.md) : Vision stratégique et chantiers en cours.

---

## 🚀 Guides de Prise en Main (Walkthroughs)
- [**Journal de Bord Général**](./walkthroughs/00_General_Walkthrough.md)
- [**Premier Pas : Orchestration d'une Scène**](./walkthroughs/Master_Storyboard_Walkthrough.md)
- [**Universal Search (Spotlight)**](./walkthroughs/2026-03-23-universal-search-spotlight.md)
- [**Social Nexus v2**](./walkthroughs/walkthrough_social_nexus_v2.md)
- [**Persistance Media Hub (Asset Lock)**](./walkthroughs/2026-03-24_Media_Persistence.md)
- [**Intégration Export Obsidian**](./walkthroughs/Obsidian_Export_Integration.md)
- [**System Forge IA (v5.1/v6 Evolution)**](./walkthroughs/System_Forge_IA_5.1.md)

---

## 📦 Modules de Session & Narration
- [**Session OS (Cockpit & Campagnes)**](./Session_OS_User_Guide.md) : *Modularisé en v6*.
- [**Map OS (Atlas & Tactique)**](./Map_OS_User_Guide.md) : *Brouillard persistant v2*.
- [**Combat OS (Gestionnaire de Batailles)**](./Combat_OS_User_Guide.md)
- [**Calculateur de Dégâts Intelligent**](./Smart_Damage_Calculator_Guide.md) : *Support multilingue v6*.
- [**Système d'Indices (Clues & Preuves)**](./Clues_User_Guide.md) : *Sync Journal v6*.
- [**NPC OS (Générateur de PNJ)**](./NPC_OS_User_Guide.md)
- [**Tablet Hub (Interface Joueurs)**](./Tablet_Hub_User_Guide.md) : *Notes privées synchronisées*.
- [**GM Remote Control (Télécommande)**](./Remote_Control_User_Guide.md)
- [**Dice-OS (Projection Automatique)**](./Dice_Projection_Guide.md)
- [**Universal Search (Spotlight)**](./Universal_Search_User_Guide.md)
- [**Table OS (Butins & Aléatoire)**](./Table_OS_User_Guide.md)
- [**Clock OS (Gestion du Temps)**](./Clock_OS_User_Guide.md)

---

## 🔉 Audio, Image & Lumière
- [**Media Hub (Gestionnaire Central)**](./Media_Hub_User_Guide.md)
- [**Music OS**](./Music_OS_User_Guide.md)
- [**Sound OS (FX Instantanés)**](./Sound_OS_User_Guide.md)
- [**Ambient OS (Mixage 8 Pistes)**](./Ambient_OS_User_Guide.md)
- [**Image OS (Projections Hub)**](./Image_OS_User_Guide.md)
- [**Voice OS (Modulateur de Voix)**](./Voice_OS_User_Guide.md)
- [**Light OS (Philips Hue)**](./Light_OS_User_Guide.md)
- [**Master Soundscape (Volume & Focus)**](./Audio_Master_Guide.md)

---

## 🛠️ Documentation Technique (Blueprints)
- [**Architecture des Modules Session-OS**](../../docs/technical/session-os-modular-architecture.md)
- [**Spécification Nexus-OS v2.0**](../../docs/blueprints/nexus_os_specification.md)
- [**Roadmap Technique v6**](../../docs/architecture/roadmap-v6.md)
- [**Remote Control Architecture**](../Technical%20Docs/Remote_Control_Technical_Doc.md)
- [**NPC Live Generator (Architecture IA)**](../Technical%20Docs/NPC_Live_Generator_Technical_Doc.md)
- [**Combat Projection (Système de Visibilité)**](../Technical%20Docs/Combat_Projection_Technical_Doc.md)
- [**Dice Projection (Architecture & Sync)**](../Technical%20Docs/Dice_Projection_Technical_Doc.md)

---

> [!NOTE]
> Cette documentation est interactive. Cliquez sur les liens pour accéder aux détails de chaque module.
