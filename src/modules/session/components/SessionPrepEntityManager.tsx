    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
                <span className="text-[9px] text-app-text/20 font-bold uppercase tracking-widest ml-1 mb-1">In Session</span>
                {activeEntities.length === 0 ? (
                    <div className="p-4 border border-dashed border-app-border rounded-xl text-center">
                        <p className="text-[10px] text-app-text/20 italic">No entities linked yet.</p>
                    </div>
                ) : (
                    <div className="flex flex-col gap-2">
                        {activeEntities.map(entity => (
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <h5 className="text-xs font-bold text-app-text/80 truncate">{entity.name}</h5>
                                    <div className="flex items-center gap-1.5 mt-0.5">
                                        {getRoleIcon(entity.role)}
                                        <span className="text-[9px] text-app-text/40 uppercase font-mono tracking-tighter">{entity.description}</span>
                                    </div>
                                </div>
                                <button 
                                    onClick={() => removeEntityFromSession(sessionId, entity.id)}
                                    className="p-1.5 text-app-text/20 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                                >
                                    <X size={16} />
                                </button>
                            </div>
            ...
            <div className="flex flex-col gap-3 pt-4 border-t border-app-border/40">
                <div className="flex items-center gap-2 bg-app-bg border border-app-border rounded-lg px-3 py-2">
                    <Search size={14} className="text-app-text/20" />
                    <input 
                        type="text" 
                        placeholder="Chercher un PNJ ou Monstre..." 
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="bg-transparent border-none text-[11px] text-app-text/60 p-0 focus:ring-0 w-full placeholder:text-app-text/10"
                    />
                </div>
                ...
                <div className="max-h-[300px] overflow-y-auto custom-scrollbar flex flex-col gap-2">
                    {availableEntities.map(entity => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-[11px] font-bold text-app-text/40 group-hover:text-app-text transition-colors truncate">{entity.name}</p>
                                <p className="text-[8px] text-app-text/20 uppercase font-bold tracking-tighter">{entity.type}</p>
                            </div>
                            <Plus size={14} className="text-app-text/20 group-hover:text-accent transition-colors ml-auto mr-1" />
                        </button>
                    ))}
                    {availableEntities.length === 0 && (
                        <p className="text-[9px] text-app-text/20 italic text-center py-2">All available entities linked.</p>
                    )}
                </div>
            </div>
        </div>
