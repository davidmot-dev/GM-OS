...
const SessionWorkspace: React.FC = () => {
    ...
    return (
        <section className="col-span-6 flex flex-col gap-6 p-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="bg-slate-950/40 backdrop-blur-2xl border-b border-white/10 flex items-center px-8 h-20 gap-8 relative shadow-2xl rounded-2xl">
                <div className="flex items-center gap-10">
                    {/* PC List */}
                    <div className="flex flex-col justify-center">
                        <span className="text-[9px] font-bold tracking-[0.3em] text-gm-gold uppercase opacity-60 mb-1 block ml-1">Party</span>
                        <div className="flex -space-x-3">
                            {(session.sessionEntityIds || []).map(id => {
                                ...
                                return (
                                    <div key={pc.id} className="w-12 h-12 rounded-full border-2 border-slate-950 bg-slate-800 relative group cursor-pointer hover:z-20 transition-all hover:scale-110">
                                        ...
                                        <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-800 border border-slate-700 rounded px-2 py-1 text-[10px] hidden group-hover:flex items-center gap-2 z-50 shadow-xl">
                                            ...
                                            <span className="font-mono font-bold text-white min-w-[20px] text-center">{pc.hp}</span>
                            ...
                            <button 
                                ...
                                className="w-12 h-12 rounded-full border-2 border-dashed border-white/10 flex items-center justify-center text-white/20 hover:text-gm-gold hover:border-gm-gold/50 transition-all group bg-slate-900/40"
                                title="Manage Party"
                            >
                    ...
                    {/* Session NPC List */}
                    <div className="flex flex-col justify-center">
                        <span className="text-[9px] font-bold tracking-[0.3em] text-gm-gold uppercase opacity-60 mb-1 block ml-1">Active NPCs</span>
                        <div className="flex gap-3">
                            {(session.sessionEntityIds || []).map(id => {
                                ...
                                return (
                                    <div 
                                        key={npc.id} 
                                        className="w-12 h-12 rounded-xl border-2 border-slate-900 bg-slate-800 relative group cursor-pointer hover:border-gm-gold/50 transition-all hover:scale-110 shadow-lg"
                                    >
                                        ...
                                        <div className={`absolute -bottom-1 -right-1 w-3 h-3 rounded-full border border-slate-950 ${npc.status === 'alive' ? 'bg-emerald-500' : npc.status === 'injured' ? 'bg-amber-500' : 'bg-red-600'}`}></div>
                            ...
                            <button 
                                ...
                                className="w-12 h-12 rounded-xl border-2 border-dashed border-white/10 flex items-center justify-center text-white/20 hover:text-gm-gold hover:border-gm-gold/50 transition-all group"
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar no-scrollbar">
                        {activeLocations.map(loc => (
                            ...
                            >
                                <div className="w-10 h-10 rounded-lg overflow-hidden border border-white/5 bg-slate-800">
                                    <ResolvedAsset 
                                        src={loc.fileUrl} 
                                        isVideo={loc.isVideo}
