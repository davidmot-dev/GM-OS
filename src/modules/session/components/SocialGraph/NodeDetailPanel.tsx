    ...
    return (
        <div className="absolute top-6 bottom-6 right-6 w-96 bg-black/40 backdrop-blur-2xl border border-white/10 rounded-3xl p-8 flex flex-col shadow-2xl animate-fade-in z-50">
            ...
            <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                    <span className="text-neonCyan text-[10px] font-black uppercase tracking-[0.3em] mb-1 block">{selectedNode.type} profile</span>
                    ...
                    <div className="mt-2">
                        {isEditingFaction ? (
                            ...
                            >
                                <Shield size={10} />
                                {selectedNode.faction || 'Définir une faction'}
                            </button>
                        )}
                    </div>
                </div>
                ...
                {!isEditing ? (
                    <section>
                        <div className="flex items-center gap-2 mb-3 text-slate-400">
                            <Info size={14} />
                            <h3 className="text-[10px] font-black uppercase tracking-widest">Relations Actives</h3>
                        </div>
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex flex-col">
                                                ...
                                                <div className="flex items-center gap-1.5 mt-1">
                                                    {isOutbound ? (
                                                        <>
                                                            <span className="text-[8px] text-slate-500 font-bold uppercase tracking-tighter">Je le considère comme</span>
                                                            <MoveRight size={10} className="text-neonCyan" />
                                                        </>
                                                    ) : (
                                                        <>
                                                            <MoveLeft size={10} className="text-accent" />
                                                            <span className="text-[8px] text-slate-500 font-bold uppercase tracking-tighter">Il me considère comme</span>
                                                        </>
                                                    )}
                                                </div>
                                            </div>
                    ...
                    <>
                        ...
                        <div className="space-y-4 py-4">
                            <div className="flex items-center gap-2 text-slate-400">
                                <Trash2 size={14} />
                                <h3 className="text-[10px] font-black uppercase tracking-widest">Gérer l'existant</h3>
                            </div>
                                        ...
                                        <div key={i} className="p-3 bg-white/5 border border-white/5 rounded-xl flex justify-between items-center group">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-[9px] uppercase tracking-wider opacity-60" style={{ color: getRelationColor(rel.type) }}>{rel.type}</div>
                                                </div>
                                            </div>
                                            <button 
                                                onClick={() => isOutbound ? onRemoveRelation(otherId, rel.type) : null}
                                                title={isOutbound ? "Supprimer la relation" : "Perception entrante (non modifiable)"}
                                                disabled={!isOutbound}
