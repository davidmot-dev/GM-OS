...
export class LootGenerator {
     ...
     */
    static generateFromTable(table: LootTable, allTables: LootTable[], iterationLimit = 5): InventoryItem[] {
                ...
                if (entry.type === 'table') {
                    ...
                    if (nextTable) {
                        ...
                        for (let i = 0; i < quantity; i++) {
                            results.push(...this.generateFromTable(nextTable, allTables, iterationLimit - 1));
                        }
                    } else {
    ...
    private static resolveQuantity(min?: number | string, max?: number | string): number {
        try {
            ...
            const minVal = parseInt(String(min)) || 1;
