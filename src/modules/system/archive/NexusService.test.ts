...
// Mock Music/Sound stores pour éviter l'init AudioContext/MusicEngine
...
describe('detectConflicts', () => {
    ...
    it('utilise entityCount de l\'état si le manifest n\'a pas de stats', () => {
        ...
        const conflicts = service.detectConflicts(
            manifestWithoutStats as unknown as NexusManifest,
            stateWith3Entities
        );
...
// ─────────────────────────────────────────────
// T9 : INTEGRATION : Round-Trip complet (Scrape -> Remap -> Inject)
// ─────────────────────────────────────────────

describe('INTEGRATION : Round-Trip complet (Scrape -> Import)', () => {
    it('préserve l\'intégrité des données après un cycle complet export/import', async () => {
        // 1. PHASE EXPORT (Scrape)
        const exportedState = service.scrapeCampaignData(CAMPAIGN_ID);
        expect(exportedState.campaign.name).toBe(mockCampaign.name);
        expect(exportedState.entities).toHaveLength(1);

        // 2. PHASE TRANSPORT (Simulation d'archiveur)
        // On simule que l'archiveur a déplacé m-avatar-001 vers assets/profiles/e-001.png
        const assetMap: Record<string, string> = {
            'm-avatar-001': '/appdata/nexus/assets/profiles/avatar_cloned.png',
            'm-map-file-001': '/appdata/nexus/assets/maps/dungeon_cloned.png',
        };

        // 3. PHASE IMPORT (Remap)
        const remappedState = service.remapPaths(exportedState, assetMap);
        expect(remappedState.entities[0].avatar).toBe(assetMap['m-avatar-001']);

        // 4. PHASE INJECTION (Inject)
        // Scénario : On remplace la campagne existante
        const { useSessionOSStore } = await import('../../session/useSessionOSStore');
        const setStateSpy = vi.mocked(useSessionOSStore.setState);

        // On appelle la méthode privée injectState via cast
        (service as unknown as { injectState: (state: NexusCampaignState) => void }).injectState(remappedState);
    ...
    it('génère un bundle valide via buildManifest', () => {
        const state = service.scrapeCampaignData(CAMPAIGN_ID);
        const assetRefs = service.collectAssetPaths(state);
        const manifest = service.buildManifest(state, assetRefs);

        expect(manifest.campaignId).toBe(CAMPAIGN_ID);
        expect(manifest.assetRefList).toContain('m-avatar-001');
        expect(manifest.schemaVersion).toBe(NEXUS_SCHEMA_VERSION);
    });
});
