...
- **Optimisation de Densité (v6.2.4)** : L'interface d'inventaire tablette utilise désormais une échelle de **50%** pour les éléments de liste. Cela permet une meilleure vue d'ensemble du paquetage sans sacrifier la lisibilité des icônes premium.
...
## 6. Suppression d'Objets (Action "Jeter")

Depuis la v6.2.4-dev, les joueurs peuvent supprimer des objets de leur inventaire.

### Flux de Suppression Optimiste
1. **Action Joueur** : Clic sur le bouton "Jeter" (Confirmation requise via dialogue UI).
2. **Mise à jour Locale (Tablette)** : L'objet est immédiatement retiré du store local via `removeInventoryItem`.
3. **Signal Réseau** : Envoi de l'événement `session:remove-inventory-item` au MJ via le Nexus Bridge.
4. **Persistance MJ** : Le `RemoteListener` du MJ intercepte l'appel et met à jour définitivement la base de données de la campagne.

---
*Dernière mise à jour : 9 Avril 2026*
*Statut : Système d'inventaire complet (Sync, Échange P2P, Suppression).*
