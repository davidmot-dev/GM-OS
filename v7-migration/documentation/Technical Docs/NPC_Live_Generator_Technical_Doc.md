...
### 4. AI Enrichment (Ollama)

Since v5.4, the generator can be enriched using local LLMs:
- **Feature**: `enrichNPCEntity` in `AIService`.
- **Logic**: The raw JSON data is sent to Ollama with the campaign context to generate descriptive text.
- **Security Layer**: A "Sanitizer" in `useNPCStore.ts` ensures all AI outputs are converted to strings and flattened to prevent React rendering crashes.

## Prompt Engineering Logic

Prompts are dynamically constructed within `useNPCStore.ts` using the entity's current fields. If AI Enrichment is enabled, **suggested prompts** are also generated by Ollama to provide higher quality visual instructions for Imagen-3.
