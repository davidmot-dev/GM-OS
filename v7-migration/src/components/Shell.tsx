...
const Shell: React.FC<ShellProps> = ({ children }) => {
    ...
    const handleLaunchHub = () => {
        ...
        if (AppBridge.session.hasSupport) {
            console.log('[Shell] Calling bridge launchHubWindow');
            gmToast(t('common:launching_hub'), 'info');
            // Note: launchHubWindow might need to be added to AppBridge.session if not already there
            if (AppBridge.session.launchHubWindow) {
                AppBridge.session.launchHubWindow();
            }
        } else {
    ...
    const handleLaunchTabletHub = () => {
        ...
        if (AppBridge.session.hasSupport) {
            ...
            if (AppBridge.session.launchHubWindow) {
                AppBridge.session.launchHubWindow('tablet'); 
            }
        } else {
