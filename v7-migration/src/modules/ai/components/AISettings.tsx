...
const AISettings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (AppBridge.mcp.hasSupport) {
      // @ts-ignore - checkStatus might be a custom method
      if (AppBridge.mcp.checkStatus) {
        // @ts-ignore
        AppBridge.mcp.checkStatus('obsidian').then((active: boolean) => setIsObsidianActive(active));
      }
    }
  ...
  useEffect(() => {
    const fetchModels = async () => {
      ...
      } else if ((activeProvider === 'ollama' || activeProvider === 'ollama_cloud') && AppBridge.ai.hasSupport) {
        setIsLoadingModels(prev => ({ ...prev, [activeProvider]: true }));
        try {
          const endpoint = configs[activeProvider].endpoint;
          const models = await AppBridge.ai.ollamaListModels(endpoint);
  ...
  const runGlobalDiagnostic = async () => {
    ...
    // 4. Test Ollama Local
    try {
      if (AppBridge.ai.hasSupport) {
        const online = await AppBridge.ai.ollamaStatus(configs.ollama.endpoint);
    ...
    try {
      if (configs.ollama_cloud.endpoint) {
        if (AppBridge.ai.hasSupport) {
          const online = await AppBridge.ai.ollamaStatus(configs.ollama_cloud.endpoint);
    ...
    try {
      if (AppBridge.mcp.hasSupport) {
        const res = await AppBridge.mcp.callTool('notebooklm', 'notebook_list', { max_results: 1 });
        if (res && res.content) {
  ...
  return (
    <div className="space-y-6">
              ...
              {p.id === 'ollama' && (
                <div className="space-y-2">
                  ...
                  <div className={`flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${
                    ...
                    <button
                      onClick={async () => {
                        const ok = await AppBridge.ai.ollamaPull('phi3', configs.ollama.endpoint);
                        if (ok) {
                          gmToast(t('ai.actions.pull_phi3_success', 'Phi-3 downloaded!'));
                          const models = await AppBridge.ai.ollamaListModels(configs.ollama.endpoint);
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (p.id === 'gemini') {
                             ...
                             models = await AppBridge.ai.ollamaListModels(configs[p.id].endpoint);
                          }
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    await AppBridge.ai.ollamaPull('gemma4:26b', configs.ollama.endpoint);
        ...
        <div className="bg-black/20 rounded-xl p-4 border border-white/5 flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={async () => {
                try {
                  const result = await AppBridge.mcp.reauthenticate();
                  if (result?.success) {
      ...
      <div className="p-4 rounded-2xl bg-gm-emerald/5 border border-gm-emerald/20 flex items-center justify-between gap-4">
        ...
        <button
          onClick={async () => {
            if (isReindexing || !AppBridge.ai.hasSupport) return;
            setIsReindexing(true);
            try {
              await AppBridge.ai.reindex();
