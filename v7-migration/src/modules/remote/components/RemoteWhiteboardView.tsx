            ...
            <div className={`flex items-center gap-2 p-2 border-t overflow-x-auto no-scrollbar justify-center ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/50 border-white/10'}`}>
                {colors.map((color) => (
                    <button
                        ...
                        className={`size-6 rounded-full border transition-transform shrink-0 bg-[var(--swatch-color)] ${currentColor === color ? 'scale-125 ring-2 ring-accent ring-offset-2 ring-offset-slate-900' : 'border-white/10 hover:scale-110'}`}
