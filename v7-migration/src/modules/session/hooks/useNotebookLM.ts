...
export const useNotebookLM = () => {
    ...
    const queryNotebook = useCallback(async (notebookId: string, query: string) => {
        ...
        try {
            ...
            if (personaPrompt) {
                try {
                    await AppBridge.mcp.callTool('notebooklm-mcp-server', 'chat_configure', {
            ...
            const response = await AppBridge.mcp.callTool('notebooklm-mcp-server', 'notebook_query', {
                notebook_id: notebookId,
                query: enrichedQuery
            });
    ...
    const reauthenticate = useCallback(async () => {
        try {
            if (AppBridge.mcp.hasSupport) {
                await AppBridge.mcp.reauthenticate();
            }
        } catch (error) {
